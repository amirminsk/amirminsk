...
body(
    ...
    'I am not allowed to use the processing functions from cv2, so I write all the '
    'filters by myself using only basic loops and math. I only use cv2 for reading '
...
body(
    ...
    'I use a median filter with 5x5 window. For each pixel I take the 25 values '
    'around it, sort them, and pick the middle one. The median is good for this '
    'because the isolated bright pixels get removed without blurring the edges. '
...
body(
    ...
    'exp(-(x^2+y^2)/(2*sigma^2)) with sigma=1.5, then I normalize it so the weights '
    'add to 1. After that I apply it by doing a weighted sum over each pixel '
    'neighborhood.'
)
body(
    'For image 3 it is more difficult because it has two kind of noise together, '
    'salt and pepper dots and also the image is grainy. So I do it in two step. '
    'First I use a 3x3 median filter to remove the dots, because median is good for '
    'impulse noise. After that I use the gaussian filter again (same like image 2) '
...
body(
    'For image 3 the result is much better after I use two steps. At first I only '
    'use one filter but it was not enough, the dots were still there. After I do the '
    'median filter first the dots are gone, and then the gaussian filter smooth the '
    'grainy part. Now the text "ABUNDANCE" is clear and readable. The disadvantage of '
    'doing two step is that it take more time to run, and the gaussian also make the '
    'image a little bit soft.'
)

heading('4 Summary')
body(
    'The most hard thing for me was image 3 because it has more than one type of '
    'noise. At first I try only one filter but it was not enough, so I had to think '
    'and combine two filters together. The median and gaussian filter for image 1 '
    'and 2 were more straightforward to implement.'
)
body(
    ...
    'filter, and when there is mixed noise we can combine them. I also understand now '
    'that writing these filters from scratch help me understand much better how they '
    'actually work compared to just calling a library function.'
)

doc.save('/home/user/amirminsk/project1_solution/report_project1.docx')
print('done')
